...
"View or modify FGACFS permissions\n"
    ...
    "\tFRW\t(W) rewrite file (modify existing content, no append)\n"
    "\tFAP\t(W) append to file (write to the end)\n"
    "\tFTR\t(W) truncate file (reduce file size, no append)\n"
    ...
    "\tDSP\t(W) chown and chgroup parent dir owner:group\n"
    ...
    "\t\tcheck for either OUS or UGR permission if calling process user\n"
    "\t\t\tis owner or in group of the object correspondingly\n"
    "\t\tcheck for calling process user permissions\n"
    "\t\tcheck for calling process group permissions\n"
    "\t\tcheck for OTH permission in none of above (except ALL) applicable"
    ...
    "\n\nInheritance flags are (dir only/both, inherit from parent rule/transmit to child rule)\n"
    "\tFPI\t(both, inherit)  inherit file permissions\n"
    "\tFPK\t(both, inherit)  keep (copy) file inherited permissions on FPI flag removing\n"
    "\tFPC\t(dirs, transmit) copy file permissions to new childs\n"
    "\tDPI\t(dirs, inherit)  inherit directory permissions\n"
    "\tDPK\t(dirs, inherit)  keep (copy) inherited directory permissions on DPI flag removing\n"
    "\tDPC\t(dirs, transmit) copy direcoty permissions to new childs\n"
    "\tSFI\t(dirs, transmit) set FPI flag to new childs\n"
    "\tSFK\t(dirs, transmit) set SFK flag to new childs\n"
    "\tSFC\t(dirs, transmit) set SFC flag to new childs\n"
    "\tSDI\t(dirs, transmit) set SDI flag to new childs\n"
    "\tSDK\t(dirs, transmit) set SDK flag to new childs\n"
    "\tSDC\t(dirs, transmit) set SDC flag to new childs\n"
